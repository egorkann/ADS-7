...
void runExperiment(int maxN, const std::string& mode, std::ofstream& opsOut, std::ofstream& timeOut) {
    ...
    for (int n = 2; n <= maxN; ++n) {
        ...
        for (int i = 0; i < n; ++i) {
            if (mode == "all_off") train.addCar(false);
            else if (mode == "all_on") train.addCar(true);
